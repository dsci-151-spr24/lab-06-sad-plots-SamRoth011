...
staff_long %>%
  ...
  geom_line() +
  labs(
    title = "Faculty Types by Year (1975-2011)",
...
***Made changes to the previous code.***
...
fisheries_plot <- fisheries %>%
  filter(total >= 800000) %>%
  mutate(country = fct_reorder(country, aquaculture, min)) %>%
  select(-total) %>%
  pivot_longer(cols = -country, names_to = "type")

  ggplot(fisheries_plot, aes(y = fct_infreq(country), x = value, fill = type)) +
    geom_bar(stat = "identity", position = "fill") +
    scale_fill_manual(values=c("red", "#3182bd")) +
    theme(legend.title = element_blank()) +
    theme(legend.position = "top") +
    labs(
      title = "Fishing by Country") +
    xlab("Proportion") +
    ylab("")
